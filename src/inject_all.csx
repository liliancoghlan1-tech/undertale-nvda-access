...
" + beats + @"
    ...
    if (keyboard_check_pressed(ord(""H"")))
    {
        ...
        else external_call(global.nvda_speak, ""No hint for what you are doing right now."");
    }
    ...
    if (instance_exists(obj_mainchara) && keyboard_check_pressed(ord(""U"")))
    {
        ...
        {
            external_call(global.nvda_speak, ""No way out found in this room."");
        }
        else
        {
            // nearest first
            ...
            var _dir = """";
            if (abs(_dy) > (abs(_dx) * 2)) { if (_dy < 0) _dir = ""north""; else _dir = ""south""; }
            else if (abs(_dx) > (abs(_dy) * 2)) { if (_dx < 0) _dir = ""west""; else _dir = ""east""; }
            else
            {
                if (_dy < 0) _dir = ""north""; else _dir = ""south"";
                if (_dx < 0) _dir += ""west""; else _dir += ""east"";
            }
            var _pace = round(point_distance(_px, _py, global.nvda_exx[_k], global.nvda_exy[_k]) / 10);
            global.nvda_sel = global.nvda_exid[_k];
            external_call(global.nvda_speak, ""Way out "" + string(_k + 1) + "" of "" + string(global.nvda_nex)
                + "", "" + _dir + "", "" + string(_pace) + "" paces. Press V to walk there, or U for the next one."");
        }
    }
}
";
...
string tag = @"
    ...
    with (obj_mettnews_artifact) { if (gtfo == 0) nvda_kind = ""Bomb, walk here to defuse""; }
...
{" + bridge + @"
    ...
    if (_m != nvda_smenu || _cur != nvda_scur)
    {
        ...
        else if (_m == 5)
        {
            ...
            if (sellpos == 8) _say = ""Exit"";
            else if (sold[sellpos] == 1) _say = ""Already sold"";
            else if (value[sellpos] == -1) _say = string(global.itemnameb[sellpos]) + "", she will not buy this"";
            else _say = string(global.itemnameb[sellpos]) + "", "" + string(value[sellpos]) + "" gold"";
            if (_enter) _say = ""Sell. "" + _say;
        }
        else if (_m == 6)
        {
            if (sellpos2 == 0) _say = ""Yes, sell for "" + string(value[sellpos]) + "" gold"";
            else _say = ""No"";
            if (_enter) _say = ""Sell "" + string(global.itemnameb[sellpos]) + "" for "" + string(value[sellpos]) + "" gold? "" + _say;
        }
        if (_say != """")
            external_call(global.nvda_speak, _say);
        nvda_smenu = _m;
...
" + HintGml("itembox", "Storage box. The left side is what you are carrying, the right side is the box. Press left and right to swap sides, up and down to move through the slots, and Z on an item to move it across to the other side.") + @"
        ...
        if (column != nvda_bcol || _row != nvda_brow)
        {
            ...
            if (column == 0)
            {
                ...
                if (global.item[_row] == 0) _nm = ""Empty"";
            }
            else
            {
                scr_storagename(boxno);
                _nm = global.itemname[_row];
                if (global.flag[boxno + _row] == 0) _nm = ""Empty"";
            }
            var _s = _nm;
            if (_ent)
            {
                if (column == 0) _s = ""Carrying. "" + _s;
                else _s = ""Box. "" + _s;
            }
...
" + HintGml("jetpack", "Jetpack climb. You rise on your own. Steer left and right with the arrow keys to dodge Mettaton's flying lights. If a light hits you it knocks you back down and costs time. A beep points to the clear side, whichever ear you hear it in is the safe way to move, and the pitch rises as a light gets close. Reach the top before the timer runs out.") + @"
        ...
        if (_secs != global.nvda_jet_sec)
        {
            ...
            if (_secs == 30 || _secs == 20 || _secs == 15 || _secs == 10 || (_secs <= 5 && _secs >= 1))
                external_call(global.nvda_speak, string(_secs) + "" seconds"");
        }

        // ---- progress milestones ----
        var _frac = (maxdist - dist) / maxdist;
        if (_frac >= 0.5 && global.nvda_jet_prog < 1) { global.nvda_jet_prog = 1; external_call(global.nvda_speak, ""Halfway up.""); }
        if (_frac >= 0.85 && global.nvda_jet_prog < 2) { global.nvda_jet_prog = 2; external_call(global.nvda_speak, ""Almost at the top.""); }
    ...
    else if (variable_instance_exists(id, ""won""))
    {
        ...
        { global.nvda_jet_result = 1; if (variable_global_exists(""nvda_ready"")) external_call(global.nvda_speak, ""You reached the top!""); }
        if (won == 2 && global.nvda_jet_result != 2)
        { global.nvda_jet_result = 2; if (variable_global_exists(""nvda_ready"")) external_call(global.nvda_speak, ""Out of time.""); }
    }
}";
...
{" + tbridge + @"
    ...
    if (instance_exists(obj_multitileevent))
    {
        ...
        if (_s != global.nvda_tile_sec)
        {
            ...
            if (_s == 20 || _s == 15 || _s == 10 || (_s <= 5 && _s >= 1))
                external_call(global.nvda_speak, string(_s) + "" seconds"");
        }
    }
    ...
    {
        nvda_t_lasto = orange;
        if (orange == 1) external_call(global.nvda_speak, ""You smell of oranges. Water bites now."");
        else if (orange == 0) external_call(global.nvda_speak, ""You smell of lemons. Water is safe now."");
    }
    // ---- piranha bite ----
    if (pir > nvda_t_lastpir) { nvda_t_lastpir = pir; external_call(global.nvda_speak, ""Ouch, piranhas!""); }
    ...
    if (nvda_t_lastmv != 0 && moving == 0)
    {
        ...
        var _nm = """";
        if (_c == 1) _nm = ""Pink"";
        else if (_c == 4) _nm = ""Yellow, zap"";
        else if (_c == 6) { if (orange == 1) _nm = ""Water, danger""; else _nm = ""Water""; }
        else if (_c == 7) _nm = ""Green"";
    ...
    if (moving == 0 && keyboard_check_pressed(ord(""Q"")))
    {
        ...
        {
            if (_dn == 0) { _dx = -10; _dy = 25; _dnm = ""Left""; }
            if (_dn == 1) { _dx = 30;  _dy = 25; _dnm = ""Right""; }
            if (_dn == 2) { _dx = 10;  _dy = 5;  _dnm = ""Up""; }
            if (_dn == 3) { _dx = 10;  _dy = 45; _dnm = ""Down""; }
            ...
            if (instance_position(_cx, _cy, obj_tilegreen)) _c = 7;
            var _p = """";
            if (_c == -1) { if (_dn == 1) _p = ""the exit""; else _p = ""edge""; }
            else if (_c == 1) _p = ""pink, safe"";
            else if (_c == 2) _p = ""red, blocked"";
            else if (_c == 3) _p = ""purple, lemons"";
            else if (_c == 4) _p = ""yellow, zap"";
            else if (_c == 5) _p = ""orange scent"";
            else if (_c == 6) { if (orange == 1) _p = ""water, piranhas""; else _p = ""water, safe""; }
            else if (_c == 7) _p = ""green"";
            _say += _dnm + "", "" + _p + "". "";
        }
        external_call(global.nvda_speak, _say);
    }
...
{" + tbridge + @"
    ...
    if (con == 75 && nvda_te_last != 75) { nvda_te_last = 75; external_call(global.nvda_speak, ""You crossed the tiles! Well done.""); }
    if (con == 30 && nvda_te_last != 30) { nvda_te_last = 30; external_call(global.nvda_speak, ""Out of time.""); }
}";
...
" + HintGml("bombs", "Bomb defusal. Six bombs are placed around this stage and you must reach and defuse every one before time runs out. Press E to scan, the bombs are announced as bombs, then press V to walk to the nearest one, or use T and R to pick which. Interact with a bomb to start defusing it. While defusing, a beep tracks the bomb piece as it sweeps from side to side and rises and speeds up as it nears the centre. Choose the defuse option the instant you hear the sharp high tone, that means the piece is in the defuse zone. Each defused bomb is removed and you will be told how many are left.") + @"
        ...
        if (global.flag[395] != global.nvda_bombs_last)
        {
            ...
            {
                if (_left > 0) external_call(global.nvda_speak, ""Bomb defused. "" + string(_left) + "" left."");
                else external_call(global.nvda_speak, ""All bombs defused! Well done."");
            }
        }
    }
}";
